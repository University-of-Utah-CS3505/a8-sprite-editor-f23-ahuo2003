...
MainWindow::MainWindow(SpriteModel &model, Canvas &canvas, QWidget *parent)
  ...
  connect(ui->drawingCanvas, &Canvas::mouseEventSignal, &model,
          &SpriteModel::useTool);
  connect(&model, &SpriteModel::updateFrame, ui->drawingCanvas,
          &Canvas::redrawCanvas);
  connect(&model, &SpriteModel::updateScaleFactor, ui->drawingCanvas,
          &Canvas::updateCanvasScaleFactor);

  // Slider Connections
  connect(ui->redSlider, &QSlider::valueChanged, this,
          &MainWindow::onSlidersValueChanged);
  connect(ui->greenSlider, &QSlider::valueChanged, this,
          &MainWindow::onSlidersValueChanged);
  connect(ui->blueSlider, &QSlider::valueChanged, this,
          &MainWindow::onSlidersValueChanged);
  connect(this, &MainWindow::changeModelCurrentColor, &model,
          &SpriteModel::changeColor);
  ...
  connect(&model, &SpriteModel::chooseColor, this,
          &MainWindow::changeSlidersColor);
  ...
  connect(&model, &SpriteModel::updateScaleFactor, &canvas,
          &Canvas::updateCanvasScaleFactor);

  // Sprite Player Connections
  connect(ui->nextButton, &QToolButton::clicked, &model,
          &SpriteModel::nextFrame);
  connect(ui->previousButton, &QToolButton::clicked, &model,
          &SpriteModel::previousFrame);
  connect(ui->playButton, &QToolButton::clicked, &model,
          &SpriteModel::startSpritePlayer);
  connect(&model, &SpriteModel::updateSpritePlayer, ui->animationPreview,
          &SpritePlayer::redrawCanvas);
  connect(ui->addFrameButton, &QPushButton::clicked, &model,
          &SpriteModel::addFrame);
  connect(ui->deleteFrameButton, &QPushButton::clicked, &model,
          &SpriteModel::removeFrame);
  connect(ui->fpsSlider, &QSlider::valueChanged, this,
          &MainWindow::onFPSValueChanged);
  connect(this, &MainWindow::changeFPS, &model,
          &SpriteModel::changeSpriteSpeed);
  ...
  connect(&model, &SpriteModel::requestLoadFilePath, this,
          &MainWindow::onRequestLoadFilePath);
...
void MainWindow::onRequestSaveFilePath() {
  QString fileName = QFileDialog::getSaveFileName(
      this, tr("Save Project"), "", tr("Sprite Sheet Project (*.ssp)"));
  if (!fileName.isEmpty())
    emit performSave(fileName);
}

void MainWindow::onRequestLoadFilePath() {
  QString fileName = QFileDialog::getOpenFileName(
      this, tr("Load Project"), "", tr("Sprite Sheet Project (*.ssp)"));
  if (!fileName.isEmpty())
    emit performLoad(fileName);
}
...
void MainWindow::applyRedFilter() { emit red(); }

void MainWindow::applyBlueFilter() { emit blue(); }

void MainWindow::applyGreenFilter() { emit green(); }

void MainWindow::applyGreyFilter() { emit grey(); }
...
void MainWindow::saveToggled() { emit save(); }

void MainWindow::loadToggled() { emit load(); }

void MainWindow::SetFileButton() {
...
void MainWindow::SetIcons() {
  ...
  ui->previousButton->setIcon(
      QIcon(":/stylesheet/res/icons/previousButton.png"));
...
void MainWindow::disableNextButton() { ui->nextButton->setEnabled(false); }
